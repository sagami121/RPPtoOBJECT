...
class RPPtoObjectApp(QMainWindow):
    ...
    def load_tracks(self, path):
        self.track_combo.clear(); self.track_combo.addItem("全トラック")
        try:
    ...
    def add_eff_ui(self):
        ...
        for pdef in EffDict[name]:
            ...
            if len(pdef) > 2 and pdef[2] == -1:
                ...
                s_ed = QLineEdit(str(pdef[1]))
                s_ed.setFixedWidth(50) 
                lay.addWidget(s_ed, row, 2)
                
                m_cb = QComboBox()
                m_cb.addItems(list(XDict.keys()))
                m_cb.setMinimumWidth(100) 
                lay.addWidget(m_cb, row, 3)
                
                e_ed = QLineEdit(str(pdef[1]))
                e_ed.setFixedWidth(50)
                lay.addWidget(e_ed, row, 4)
    ...
    def run_process(self):
            try:
                ...
                bez_str = self.bezier_ui.bezier_str if hasattr(self, 'bezier_ui') else "0|0,0,1,1"
                ...
                for item_num, o in enumerate(objs_to_process):
                    ...
                    if self.cb_auto_speed.isChecked():
                        ...
                        speed_candidates = [50.0, 100.0, 200.0, 400.0, 800.0]
                        speed = min(speed_candidates, key=lambda x: abs(x - raw_speed))
                    ...
                    for eff in self.added_effects_data:
                        ...
                        for p in eff["params"]:
                            if p["type"] == "cb":
                                output.append(f"{p['name']}={int(p['obj'].isChecked())}\n")
                            else:
                                meth = str(XDict.get(p["method"].currentText(), ""))
                                try:
                                    s_val = f"{float(p['start'].text()):.3f}"
                                    e_val = f"{float(p['end'].text()):.3f}"
                                except:
                                    s_val, e_val = p['start'].text(), p['end'].text()
                                
                                if meth != "":
                                    output.append(f"{p['name']}={s_val},{e_val},{meth},{bez_str}\n")
                                else:
                                    output.append(f"{p['name']}={s_val}\n")
                        p_idx += 1
                    ...
                    if self.cb_time_ctrl.isChecked():
                        tc_meth = "直線移動(時間制御)"
                        s_val = "100.000" if item_num % 2 != 0 else "0.000"
                        e_val = "0.000" if item_num % 2 != 0 else "100.000"
                        
                        if self.cb_apply_easing.isChecked():
                            time_val = f"{s_val},{e_val},{tc_meth},{bez_str}"
                        else:
                            time_val = f"{s_val},{e_val},{tc_meth},0"
                ...
                QMessageBox.information(self, "完了", f"オブジェクト出力完了\n{exo_p}")
